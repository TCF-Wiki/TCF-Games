			...
			<div class="options">
				...
				<div class="raritySlider">
					<div class="label left">
						...
						<img :src="`/loadout-images/Armor/${rarityInfo[Object.keys(rarityInfo)[minRarity]]}.png`" />
					</div>
					<div class="slider">
					...
					<div class="label">
						<img :src="`/loadout-images/Armor/${rarityInfo[Object.keys(rarityInfo)[maxRarity]]}.png`" />
						<label>
							Max: <br />
...
export default defineComponent({
	data() {
		return {
			...
			rarityInfo: {
				Common: "Basic_Helmet",
				Uncommon: "Standard_Helmet",
				Rare: "Reinforced_Helmet",
				Epic: "Combat_Helmet",
				Exotic: "Enhanced_Helmet",
				Legendary: "NVG_Helmet"
			} as any
		};
	},
	methods: {
		RandomLoadout: async function () {
			this.ResetLoadout();
			let newData = await GenerateRandomLoadout(this.minRarity, this.maxRarity, this.consumableAmount, this.alwaysGetWeapons, this.alwaysGetArmor);
		...
		getLoadoutFromURL: async function () {
			...
			if (weapons.length == 0 && items.length == 0) {
				...
				for (let i = 0; i < weapons.length; i++) {
					...
					this.weapons.push({img: weapon, ign: weapon, rarity: await GetRarity(weapon)});
				}
				for (let i = 0; i < items.length; i++) {
					...
					this.items.push({img: item, amount: parseInt(itemNumbers[i]), ign: ign, rarity: await GetRarity(item.split("/")[1])});
				}
				if (helmet) this.helmet = {img: helmet, ign: helmet.replace("Helmet_", "").split("_").join(" ") + " Helmet", rarity: await GetRarity(helmet)};
				if (shield) this.shield = {img: shield, ign: shield.replace("Shield_", "").split("_").join(" ") + " Shield", rarity: await GetRarity(shield)};
				if (backpack) this.backpack = {img: backpack, ign: backpack.split("_").join(" ") + " Backpack", rarity: await GetRarity(backpack)};
			}
		}
	},
	watch: {
