  ...
  methods: {
    StartGameFromOptions(
      ...
      if (recievedOptions) {
        ...
        options.difficulty =
          recievedOptions.difficulty || this.gameOptions.difficulty;
        options.timeLimit =
          recievedOptions.timeLimit || this.gameOptions.timeLimit;
      ...
      //Generate game seed
      let seed = `${options.length}-${options.difficulty}-${
        options.timeLimit
      }-${options.maps.includes(1) ? "1" : "0"}${
        options.maps.includes(2) ? "1" : "0"
      }${options.maps.includes(3) ? "1" : "0"}`;

      //Generate game locations
      let availableLocations: locationType[] = locationData.filter(
        (a) =>
          a.difficulty.includes(options.difficulty) &&
          options.maps.includes(a.map)
      );
